...
class Line(object):
    ...
    def __init__(self, start, end):
        ...
        self.end = end.clone()
        self.clone_number = 0
        self.static_start = self.start.clone()
        self.static_end = self.end.clone()
    ...
    def line_plus(self, other_line):
        ...
        Type hints:
          ...
          :rtype: Line:
        """
        combined_start_x = (self.start.x + other_line.start.x)
        combined_start_y = (self.start.y + other_line.start.y)
        combined_end_x = (self.end.x + other_line.end.x)
        combined_end_y = (self.end.y + other_line.end.y)
        return Line(Point(combined_start_x, combined_start_y), Point(combined_end_x, combined_end_y))
        # --------------------------------------------------------------
        # Done: 9.
        #   a. READ the above specification, including the Example.
        #        ** ASK QUESTIONS AS NEEDED. **
    ...
    def midpoint(self):
        ...
        Type hints:
          :rtype: Point
        """
        average_x = (self.end.x + self.start.x) / 2
        average_y = (self.end.y + self.start.y) / 2
